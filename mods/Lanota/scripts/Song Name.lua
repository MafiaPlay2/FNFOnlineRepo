...
local removeHypens = true -- get rid of hypens from the song name | This is ONLY for the song name that shows on the bar
    local captialization = true -- Captialize the first letter from the song name | No real need if you don't use remove hypen
...
local nowPlaynColor = 'ada163'
local credColor = 'ffed92'
...
function onCreate()
	...
	makeLuaSprite('bgThing', 'songBG', -550, 370)
    ...
    if count < 20 then
        makeLuaText('songText', "".. useName, 400, getProperty('bgThing.x') + 0, 400) -- 20 letters is the width limit
        ...
        makeIt = true
        makeLuaText('songText', "".. useName, 400, getProperty('bgThing.x') + 0, 380) -- Too big song name
        setObjectCamera("songText", 'other');
        setTextColor('songText', '0x'..songTxtColor)
        setTextSize('songText', 30);
        addLuaText("songText");
    ...
    if makeIt == true then -- too big of a song name
        makeLuaText('optionalText', 400, getProperty('bgThing.x') + 0, 335)
    ...
    --makeLuaText('beforeSongText', "Now Playing... ", 300, getProperty('bgThing.x') + 0 - 40, 380)
    ...
    if showCredit == true then
        makeLuaText('whoDuntIt', "by ".. name, 350, getProperty('bgThing.x') - 0, 440)
...
function shrank(diff)
    ...
    if diff == 'MASTER' then -- all this shit is unused anyways who cares
        return 'MT.'
    elseif diff == 'ULTRA' then
        return 'UT.'
    elseif diff == 'ACOUSTIC' then
        return 'AC.'
    elseif diff == 'WHISPER' then
        return 'WS.'
    else
